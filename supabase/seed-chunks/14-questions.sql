-- chunk 14/14 questions 8 (산업안전기사)
...
insert into public.question_answers (question_id, correct_option_id, answer_text, explanation, concepts) values
  ('ba666b73-8998-5f9e-58a1-7093f47f5dba', '69a863a6-dd9e-069a-c515-6f24a4028061', '④ 불연성 가스로서, 허용농도가 10[ppm]이다.', '정답 ④. K9 일산화탄소는 허용농도가 30[ppm]인 독성 가스이자. 공기 중 연 소범위가 12.5~74[vol%]인 가연성 가스이다. Wil리', array['산업안전기사','2019-3'])
...
on conflict (id) do update set body = excluded.body, sort_order = excluded.sort_order;
insert into public.question_answers (question_id, correct_option_id, answer_text, explanation, concepts) values
  ('ef841e56-905e-5bb4-cd53-0c54b62c01d6', '2263699f-1b48-f7a3-c75a-77bf58aec755', '④ 용기의 온도를 65도 이하로 유지한다.', '정답 ④. K3 금속의 용접 용단 또는 가열에 사용되는 가스 등의 용기를 취급 하는 경우에는 용기의 온도를 40[’C] 이하로 유지하여야 한다. &S!gi', array['산업안전기사','2019-3'])
on conflict (question_id) do update set
  correct_option_id = excluded.correct_option_id,
  answer_text = excluded.answer_text,
  explanation = excluded.explanation,
...
on conflict (id) do update set body = excluded.body, sort_order = excluded.sort_order;
insert into public.question_answers (question_id, correct_option_id, answer_text, explanation, concepts) values
  ('816d73b6-6caa-8481-cd81-c4dd3e05fc26', 'b689d04a-28a0-b578-911e-1a97f215e8ae', '② 3개', '정답 ②. KOi 동바리로 사용하는 파이프서포트를 3개 이상 이어서 사용하지 않아야 한다', array['산업안전기사','2019-3'])
on conflict (question_id) do update set
  correct_option_id = excluded.correct_option_id,
  answer_text = excluded.answer_text,
  explanation = excluded.explanation,
...
on conflict (id) do update set body = excluded.body, sort_order = excluded.sort_order;
insert into public.question_answers (question_id, correct_option_id, answer_text, explanation, concepts) values
  ('be2a3e14-e434-f4d2-0c64-def2739fae24', 'dcb608c6-2c62-34e0-1c98-769180417fb5', '② 운전반경 내에 사람이 있을 때 회전은 10[rpm] 정도의', '정답 ②. 는 사전에 회전반경, 높이제한 등 방호조치를 강구한다. 굴착기계 운행 시 운전반경 내에 사람이 있어서는 안 된다. WEI8I 아仏PTER 04 건설현장 안전시설 관리', array['산업안전기사','2019-3'])
on conflict (question_id) do update set
  correct_option_id = excluded.correct_option_id,
  answer_text = excluded.answer_text,
  explanation = excluded.explanation,
...
on conflict (id) do update set body = excluded.body, sort_order = excluded.sort_order;
insert into public.question_answers (question_id, correct_option_id, answer_text, explanation, concepts) values
  ('d39438a4-8198-053b-f3aa-d0931339392e', '7b60c20c-3deb-ef87-a1ea-6eee158150f2', '① 높이가 20[m]를 초과하거나 중량물의 적재를 수반하는', '정답 ①. 다 수평재를 설치할 것 K3 강관틀비계를 조립하여 사용하는 경우 높이가 20[m]를 초과하 거나 중량물의 적재를 수반하는 작업을 할 경우에는 주틀 간의 간격을 1.8[m] 이하로 하여야 한다. 휴 旧', array['산업안전기사','2019-3'])
on conflict (question_id) do update set
  correct_option_id = excluded.correct_option_id,
  answer_text = excluded.answer_text,
  explanation = excluded.explanation,
...
on conflict (id) do update set body = excluded.body, sort_order = excluded.sort_order;
insert into public.question_answers (question_id, correct_option_id, answer_text, explanation, concepts) values
  ('03568d74-38ad-77ba-d1a4-2af192f9ad61', 'c1a176b6-d7af-c658-4590-9f21e63763c2', '② 경보장치 설치', '정답 ②. K1H 차량계 하역운반기계 전도 등의 방지 • 유도자 배치 • 지반의 부동침하 방지 • 갓길의 붕괴 방지', array['산업안전기사','2019-3'])
on conflict (question_id) do update set
  correct_option_id = excluded.correct_option_id,
  answer_text = excluded.answer_text,
  explanation = excluded.explanation,
